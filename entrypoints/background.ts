...
export default defineBackground({
  main() {
    const updateContextMenus = async () => {
      ...
      const { chatGPTSettings = [], geminiSettings = [] } = await storage.getItems([
        'chatGPTSettings',
        'geminiSettings',
      ])
    ...
    browser.contextMenus.onClicked.addListener(async (info, tab) => {
      if (!tab?.id || typeof info.menuItemId !== 'string' || !info.selectionText) return
      ...
      if (info.menuItemId.startsWith(MENU_CHATGPT)) {
        ...
        const url = setting.gpt ? `${CHATGPT_URL}g/${setting.gpt}` : CHATGPT_URL
