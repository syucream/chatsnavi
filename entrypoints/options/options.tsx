...
const ChatGPTSettingsTable: React.FC<{
  ...
  const handleChange = (index: number, field: keyof ChatGPTSetting, value: string | boolean) => {
          ...
          <tr key={index}>
            ...
            <td>
              <input
                ...
                onChange={(e) => handleChange(index, 'autoSubmit', e.target.checked)}
              />
            </td>
            <td>
              <input
                ...
                onChange={(e) => handleChange(index, 'activate', e.target.checked)}
              />
            </td>
            <td>
              <Input
...
const GeminiSettingsTable: React.FC<{
  ...
  const handleChange = (index: number, field: keyof GeminiSetting, value: string | boolean) => {
    const newSettings = settings.map((setting, i) => {
      if (i === index) {
        return { ...setting, [field]: value }
      }
...
export const Options: React.FC = () => {
  ...
  useEffect(() => {
    storage.getItems(['local:chatGPTSettings', 'local:geminiSettings']).then((items) => {
  ...
  const handleAppendChatGPT = () => {
    setChatGPTSettings([...chatGPTSettings, { name: '', autoSubmit: false, activate: false }])
  }

  const handleAppendGemini = () => {
    setGeminiSettings([...geminiSettings, { name: '', autoSubmit: false, activate: false }])
  }
  ...
  return (
    <Container>
      ...
      <div>
        ...
        <div>
          <ChatGPTSettingsTable settings={chatGPTSettings} onSettingsChange={setChatGPTSettings} />
          <button onClick={handleAppendChatGPT}>Append a new ChatGPT setting</button>
        </div>

        <h2>Gemini settings</h2>
        <div>
          <GeminiSettingsTable settings={geminiSettings} onSettingsChange={setGeminiSettings} />
          <button onClick={handleAppendGemini}>Append a new Gemini setting</button>
        </div>
      </div>
